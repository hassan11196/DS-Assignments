...
int path_finder(dyn_2d_safe_arr<maze_block> &obj){
	...
	while(!q.is_empty()){
		...
		maze_block** possible = temp_block.vicinty(obj,maze_block::is_path_1);
		int flag= 0;
		...
		for(int i = 0; i < 4; i++){
			if(possible[i] != NULL){
				...
				flag=1;
				if(!obj(possible[i]->row,possible[i]->col).is_end())
					obj(possible[i]->row,possible[i]->col).visit();
			}
		}
		if(flag==0){
