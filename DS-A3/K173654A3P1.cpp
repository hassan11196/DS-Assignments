...
int main(){
	...
	string str_case = "case" + to_string(test_case);
	string dir = "test cases\\" + str_case;
	string temp =  dir + "\\manager"; // Change file name as required.
	...
	while(type==0){
		...
		if(type==2){
			...
			cout << endl << "First Opened File : " << gen+1 << endl;
	...
	*/
	...
	<< gen+1 << " is read first and then the rest" << endl;
	
	BST<prog_time> tree;
	BST<prog_time> conflict_tree;
	queue<prog_time> conflict_queue;
	//q.display_queue();
	q.sort_heap_except_first();
	//q.sort_except_first();
	//q.display_queue();
	//q.display_queue();
	while(!q.is_empty()){
		...
		//cout << "-> "<< temp << endl;
		if(!tree.insert_prog(temp)){
			conflict_tree.insert_force(temp);
			conflict_queue.push(temp);
		}
		q.pop();
	}
	//q.display_queue();
